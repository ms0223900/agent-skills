...
1. **Commits**：`ticket|hash|date|subject`（date 為 committer date）
2. **TICKETS**：已去重的工單清單（含僅 chore 的工單）

若 `TICKETS` 為 `(none — no matching branches in this range)`，代表區間內無符合分支。
