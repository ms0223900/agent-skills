...
- 依 reference.md 的收尾判斷規則，若這是 README 驅動型且回填後所有 P0（必要）任務皆已勾選（P1/P2 依註記可延後不計）→ 告知使用者「這個 epic/feature 看起來已收尾」，建議之後另外執行 `/distill-playbook`；也可視情況執行 `/comment-trim`，回頭掃描整個 epic 累積下來的註解贅述。**不要**自己去動 playbook / CLAUDE.md / 其他 skill 檔案，那是 `/distill-playbook` 自己的職責範圍；也不要自己動手精簡註解，那是 `/comment-trim` 的職責範圍。
