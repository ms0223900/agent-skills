---
name: next-task
description: 依目前 git branch（或指定的 JIRA 單號）自動找出對應追蹤目錄中「依賴順序上下一個未完成的任務」，依任務類型分派至測試撰寫（Vue 2 元件整合測試用 `/vue-integration-test`）、`/refactor`（提取/重構/拆分/搬移）、`/fix`（修正既有錯誤/bug）或 `/feature`（一般功能）實作，跑專案測試 runner 到全過（過程中若出現 lint/type/test/build 錯誤，交由 `/fix` 診斷修正），再呼叫 `/us-acceptance-check` 驗收該任務並回填追蹤目錄 README 的「全域驗收 Checklist」。每次只處理一個任務，不會連續跑完整個 Phase。使用時機：使用者說「接下來要做什麼」、「下一個任務」、「next task」、「這個 branch 還有哪些沒做完」、「幫我把下一個 US/TASK 做完」。
---
...
| 判斷依據 | 分類 | 分派 |
|---|---|---|
| 含「測試」，且不含「提取」/「重構」/「拆分」/「搬移」 | 純測試任務 | 先偵測技術棧：若為 **Vue 2 元件整合測試** → 依 `/vue-integration-test` 撰寫；其餘（Nuxt/Next/React、純函式/utils）→ 依專案既有測試慣例直接撰寫。**不要**呼叫 `/feature` 或 `/refactor` |
