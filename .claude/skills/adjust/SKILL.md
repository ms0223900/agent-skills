---
name: adjust
description: 用於補充調整既有功能：先更新 US、再補測試、再實作、跑測試直到全過、最後驗收 US。適用 Vue、Nuxt、Next.js 等前端專案。使用時機：使用者提出補充需求或功能調整，例如「補一下 XXX」、「調整 XXX 的行為」、「這邊需要多支援 YYY」。
---
...
1. **尋找現有測試檔**：搜尋與被修改功能相關的 `*.test.*` / `*.spec.*`（如 `.ts`、`.js`、`.tsx`、`.jsx`）。
   - 若存在 → 在現有測試檔中補充對應的 test case。
   - 若不存在 → 在功能檔案旁新建單元測試檔（依專案慣例命名，如 `{filename}.test.ts`）。
2. **測試範圍原則**：
   - 只測試本次調整涉及的功能邏輯。
...
| Playwright | `npx playwright test {測試檔路徑}`（僅當 US 明確要求 E2E） |
